...
interface ApiRecordRepository: JpaRepository<ApiRecord, ApiRecordId> {
    ...
    @Query("""
        SELECT NEW io.rss.openapiboard.server.persistence.ApiVersionResponse(a.name, a.version) 
        FROM ApiRecord a
        WHERE a.namespace = ?1
    """)
    fun findApiListByNamespace(namespace: String): List<ApiVersionResponse>
